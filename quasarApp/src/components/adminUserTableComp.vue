  ...
  <div>
    ...
    >
      <template slot="thumbnail_image" slot-scope="text, record, index">
        <img
          v-bind:src="record.thumbnail_image == null? '../assets/images/default-user-image.jpg': record.thumbnail_image"
          alt
          class="user-image"
        >
      </template>
    ...
    <a-modal title="유저 정보 수정" v-model="modalVisible">
      <template slot="footer">
        <a-popconfirm
          title="정말로 이 유저를 삭제하시겠습니까?"
          @confirm="deleteUser"
        ...
        <a-button key="submit" type="primary" :loading="loading" @click="changeUserSubmit">확인</a-button>
      </template>
      <div class="row-div popup">
        <div class="profile-div">
          <img
            v-bind:src="changeUserInfo.thumbnail_image == null? '../assets/images/default-user-image.jpg': changeUserInfo.thumbnail_image"
            alt
          >
        </div>
        <div class="content-div">
          ...
          <div class="components-input-demo-presuffix">
            ...
            <a-select v-model="changeUserInfo.car_type" @change="changeCarType">
              <a-select-option
                v-for="(currentCar, index) in carList"
                v-bind:key="index"
                v-model="currentCar.carName"
              >{{currentCar.carName}}</a-select-option>
            </a-select>
          </div>
          <div class="components-input-demo-presuffix">
  ...
  methods: {
    ...
    fetch() {
      ...
      this.$store.dispatch("database/selectTable", {
        tableName: "users",
        fields:
          "id,password,car_number,phone_number,user_name,car_type,car_km,user_level,thumbnail_image,profile_image,login_type",
        cb: function(data) {
          console.log(data);
    ...
    showModal(index) {
      console.log(index);
      var index = index + 1;
      let currentUser = this.userList.filter(item => {
    ...
    changeUserSubmit() {
      ...
      vueObj.$store.dispatch("database/updateUser", {
        ...
        car_number:
          vueObj.changeUserInfo.car_number == null
            ? ""
            : vueObj.changeUserInfo.car_number,
        password:
          vueObj.changeUserInfo.password == null
            ? ""
            : vueObj.changeUserInfo.password,
        phone_number:
          vueObj.changeUserInfo.phone_number == null
            ? ""
            : vueObj.changeUserInfo.phone_number,
        user_name:
          vueObj.changeUserInfo.user_name == null
            ? ""
            : vueObj.changeUserInfo.user_name,
        car_type:
          vueObj.changeUserInfo.car_type == null
            ? ""
            : vueObj.changeUserInfo.car_type,
        car_km:
          vueObj.changeUserInfo.car_km == null
            ? ""
            : vueObj.changeUserInfo.car_km,
        thumbnail_image:
          vueObj.changeUserInfo.thumbnail_image == null
            ? null
            : vueObj.changeUserInfo.thumbnail_image,
        profile_image:
          vueObj.changeUserInfo.profile_image == null
            ? null
            : vueObj.changeUserInfo.profile_image,
        user_level:
          vueObj.changeUserInfo.user_level == null
            ? ""
            : vueObj.changeUserInfo.user_level,
        login_type:
          vueObj.changeUserInfo.login_type == null
            ? ""
            : vueObj.changeUserInfo.login_type,
        cb: data => {
          if (data == "success") {
    ...
    deleteUser(e) {
      ...
      var deleteUser = this.userList.filter(currentUser => {
        return currentUser.id == changeUserId;
      })[0];
      ...
      this.$store.dispatch("database/deleteUser", {
        ...
        cb: data => {
          if (data == "success") {
            ...
            let users = vueObj.userList.filter(currentUser => {
              return currentUser.id != changeUserId;
            });
